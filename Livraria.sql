...
select count (idLivro) as 'Quantidade de Livros' from Livro;

select * from Cliente order by nome asc;

select Editora.nome as 'Nome da Editora', Livro.titulo as 'Título do Livro' from Editora
left join Livro on Editora.idEditora = Livro.idEditora order by Editora.nome desc;

select Editora.nome as 'Nome da Editora', round(avg(Livro.preco), 2) as 'Média de Preço dos Livros' from Livro 
inner join Editora on Livro.idEditora = Editora.idEditora group by Editora.nome;

select Cliente.nome as 'Cliente', count(ItemPedido.idLivro) as 'Quantidade de livros comprados' from Cliente
left join Pedido on Cliente.idCliente = Pedido.idCliente
left join ItemPedido on Pedido.idPedido = ItemPedido.idPedido group by Cliente.nome;
